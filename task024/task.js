...
var addNode_button=$("#addNode");
var delNode_button=$("#delNode");
//类jq选择器
function $(id){
...
function getNode(ele){
    addEventHandler(ele,'click',function(e){
        ...
        if(!e){
            ...
            if(tempKey==null){
                tempKey=e.target;
                tempKey.style.backgroundColor="yellow";
            }else {
                tempKey.style.backgroundColor = "#fff";
                tempKey=e.target;
                tempKey.style.backgroundColor="yellow";
            }
...
function addNode(){
    ...
    if(nodeValue && nodeValue!=='') {
        ...
        tempNode.textContent = nodeValue;
        tempKey.appendChild(tempNode);
    }else{
        alert("请填写您要添加节点的值");
    }
...
function delNode(){
    if(!tempKey){
        ...
        var nodeParent=tempKey.parentNode;
        nodeParent.removeChild(tempKey);
    }
}
//初始化函数
function init(event){
...
//绑定事件
addEventHandler(wrapper,'mouseover',function(){
    getNode(wrapper);
});
addEventHandler(addNode_button,"click",addNode);
addEventHandler(delNode_button,"click",delNode);
